...
- /admin/devices returns the device-centric fields
...
def main():
    ...
    # 2. Admin generates an invitation token for DEVICE001
    r = requests.post(f"{BASE}/admin/generate-token",
                      json={"device_id": DEVICE}, headers=ADMIN)
    ...
    # 7. /admin/devices returns the device-centric fields
    r = requests.get(f"{BASE}/admin/devices", headers=ADMIN)
    first = r.json()[0] if r.status_code == 200 and r.json() else {}
    all_ok &= check("admin/devices has location/device_id/company_email fields",
                    all(k in first for k in ("location", "device_id", "company_email",
